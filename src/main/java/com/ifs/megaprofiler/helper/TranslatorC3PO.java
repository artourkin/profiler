...
public class TranslatorC3PO {
    ...
    private static String[] C3PO_PROFILE_PROPERTIES = {"format", "mimetype", "puid", "format_version"};
    ...
    public static Document run(com.ifs.megaprofiler.elements.Document document) {
        ...
        root.add(new DefaultAttribute("xmlns", "http://ifs.tuwien.ac.at/dp/c3po"));
        root.add(new DefaultAttribute("xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance"));

        root.add(new DefaultAttribute("collection", "test"));
        root.add(new DefaultAttribute("date", new Date().toString()));
        root.add(new DefaultAttribute("count", Long.toString(document.getRoot().count)));
        ...
        for (Node node : nodes) {
            if (isIdentificationConflicted(node, root)) {
                ...
                if (dictionary.containsKey(key)) {
                    ...
                    dictionary.put(entry.getKey(), entry.getValue() + node.count);
                } else {
                    dictionary.put(node.getProperty(identityName).value, node.count);
                }
    ...
    static Element processValueProperty(Node root, String propertyName) {
        ...
        long conflictCount = 0;
        for (Node node : nodes) {
            if (isIdentificationConflicted(node, root) || (propertyName.equals("format_version") && isVersionConflicted(node, root))) {
                conflictCount += node.count;
            } else {
    ...
    static boolean isIdentificationConflicted(Node node, Node root) { //checking whether node a conflicted identification or a conflicted version
        while (node != null) {
        ...
        if (node != null && node.getProperty("status") != null && node.getProperty("status").value.equals("CONFLICT")) {
            return true;
        }
        return false;
    }

    static boolean isVersionConflicted(Node node, Node root) { //checking whether node a conflicted identification or a conflicted version
        if (node != null && node.getProperty("status") != null && node.getProperty("status").value.equals("CONFLICT")) {
            return true;
        }
        return false;
