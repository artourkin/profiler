...
public class Maths {
    ...
    private static void updateStats(Node root) {
        ...
        long max = -1;
        for (Node node : size) {
            if (node.getProperty("min").getValue().equals("")) {
                node.getProperty("min").setValue(node.value);
                node.getProperty("max").setValue(node.value);
                node.getProperty("avg").setValue(node.value);
            }
            long tmp = Long.parseLong(node.getProperty("max").getValue());
            if (tmp > max) {
                max = tmp;
            }
            tmp = Long.parseLong(node.getProperty("min").getValue());
            if (tmp < min) {
                min = tmp;
            }
